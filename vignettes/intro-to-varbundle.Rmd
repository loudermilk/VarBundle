...
VarBundles make it easy for R developers to bundle multiple, conceptually related read-only variables -- in essence, a named, list-like object of unmutable constants. 

### Rationale and Approach
The VarBundle package grew out of frustration with perceived shortcomings in the R language that hinder its utility for developing robust, easily refactorable and debuggable code written in a multi-developer environment.
...
VarBundles make it easy to encode and store system configuration information. "Simple" VarBundles are VarBundles that consist of only atomic, scalar values (i.e. no "complex" items such as lists or data.frames).
```{r}
...
new_vb
```
Complex VarBundle objects return `NULL` if `as.data.frame` is called.
```{r}
...
#### length
Method `$length()` - Returns the number of variables in a VarBundle
```{r}
vb <- varbundle(list(a = 1, b = 2, c = 3))
vb$length()
```
